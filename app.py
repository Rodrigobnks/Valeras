...
import os
import re
import unicodedataimport base64
import json
import re
import unicodedata
...
def cargar_categoria_dispersion(path: str) -> pd.DataFrame:
    ...
    # Evita que encabezados duplicados provoquen que df["Marca"] devuelva
    # un DataFrame en lugar de una Serie.
    ...
    # Después del renombrado también pueden quedar nombres duplicados
    # si el archivo traía dos columnas equivalentes.
    df = df.loc[:, ~df.columns.duplicated()].copy()
    ...
    if faltantes:
        raise ValueError(
            "El archivo de categorías no tiene estas columnas: "
            + ", ".join(faltantes)
        )

    columnas = requeridas + [
        c for c in ["Subdirección", "Zona", "Sucursal"]
        if c in df.columns
    ]
    df = df[columnas].copy()
...
def _agrupar_chatbot_local(
    df: pd.DataFrame | None,
    nivel: str = "Sucursal",
) -> pd.DataFrame:
    """Prepara una tabla ejecutiva para responder sin utilizar una API."""
    ...
    metricas = [c for c in metricas_suma if c in base.columns]
    for col in metricas:
    ...
    resultado = base.groupby(nivel, as_index=False)[metricas].sum()

    if {
        "Distribuidoras Totales",
        "Distribuidoras al Corriente",
    }.issubset(resultado.columns):
        resultado["Calidad de Cartera"] = resultado.apply(
            lambda r: (
                float(r["Distribuidoras al Corriente"])
                / float(r["Distribuidoras Totales"])
                * 100
            )
            if float(r["Distribuidoras Totales"])
            else 0.0,
            axis=1,
        )

        resultado["Porcentaje en Mora"] = resultado.apply(
            lambda r: (
                float(r.get("Distribuidoras en Mora", 0))
                / float(r["Distribuidoras Totales"])
                * 100
            )
            if float(r["Distribuidoras Totales"])
    ...
    if {"Total Dispersado", "Canjes"}.issubset(resultado.columns):
        resultado["Canje Promedio"] = resultado.apply(
            lambda r: (
                float(r["Total Dispersado"]) / float(r["Canjes"])
            )
            if float(r["Canjes"])
            else 0.0,
            axis=1,
    ...
    # Puntaje transparente para priorizar atención.
    calidad = resultado.get("Calidad de Cartera", pd.Series(100.0, index=resultado.index))
    pct_mora = resultado.get("Porcentaje en Mora", pd.Series(0.0, index=resultado.index))
    var_corriente = resultado.get("Var Dist Corriente", pd.Series(0.0, index=resultado.index))
    var_mora = resultado.get("Var Dist en Mora", pd.Series(0.0, index=resultado.index))

    resultado["Puntaje de Atención"] = (
        pct_mora.clip(lower=0) * 0.45
        + (100 - calidad).clip(lower=0) * 0.30
        + (-var_corriente).clip(lower=0) * 0.15
        + var_mora.clip(lower=0) * 0.10
    )
...
def _formato_chat_numero(valor: float) -> str:
    return f"{float(valor):,.0f}"
...
def _fila_chatbot(
    ...
    if serie.dropna().empty:
        return None
    indice = serie.idxmax() if mayor else serie.idxmin()
...
def _explicar_prioridad_local(fila: pd.Series, nivel: str) -> str:
    ...
    totales = float(fila.get("Distribuidoras Totales", 0))
    calidad = float(fila.get("Calidad de Cartera", 0))
    var_corriente = float(fila.get("Var Dist Corriente", 0))
    var_mora = float(fila.get("Var Dist en Mora", 0))
    pct_mora = float(fila.get("Porcentaje en Mora", 0))

    motivos = []
    if mora:
        motivos.append(
            f"concentra {_formato_chat_numero(mora)} distribuidoras en mora "
            f"({pct_mora:,.1f}% de su base)"
        )
    if calidad:
        motivos.append(f"su calidad de cartera es {calidad:,.2f}%")
    if var_corriente < 0:
        motivos.append(
            f"perdió {_formato_chat_numero(abs(var_corriente))} distribuidoras al corriente"
        )
    elif var_corriente > 0:
        motivos.append(
            f"ganó {_formato_chat_numero(var_corriente)} distribuidoras al corriente"
        )
    if var_mora > 0:
        motivos.append(
            f"la mora aumentó en {_formato_chat_numero(var_mora)} distribuidoras"
        )

    if not motivos:
        motivos.append("presenta el mayor puntaje combinado de atención dentro del alcance")

    return (
        f"**{nombre}** requiere la mayor atención porque "
        + ", ".join(motivos)
        + ". La prioridad sugerida es reforzar cobranza y seguimiento puntual, "
          "antes de acelerar nueva colocación."
    )


def _respuesta_categoria_local(pregunta: str) -> str:
    texto = limpiar_texto(pregunta)
    numero = re.search(r"\$?\s*([\d,]+(?:\.\d+)?)", pregunta)
    if numero:
        colocado = float(numero.group(1).replace(",", ""))
        if colocado <= 40000:
            categoria = "Bronce"
        elif colocado <= 60000:
            categoria = "Plata"
        elif colocado <= 100000:
            categoria = "Oro"
        elif colocado <= 150000:
            categoria = "Platino"
        elif colocado <= 220000:
            categoria = "Diamante"
        else:
            categoria = "Embajadoras"

        return (
            f"Con un colocado de **{_formato_chat_moneda(colocado)}**, "
            f"la distribuidora corresponde a la categoría **{categoria}**. "
            "La categoría indica capacidad comercial, pero debe evaluarse junto "
            "con calidad de cartera, mora y comportamiento de pago."
        )

    return (
        "Las categorías por colocado son: **Bronce** hasta $40,000; "
        "**Plata** de $40,001 a $60,000; **Oro** de $60,001 a $100,000; "
        "**Platino** de $100,001 a $150,000; **Diamante** de $150,001 a $220,000; "
        "y **Embajadoras** de $220,001 a $500,000."
    )


def _respuesta_comparacion_local(
    tabla: pd.DataFrame,
    nivel: str,
    nombres: list[str],
) -> str:
    ...
    lineas = [
        f"Comparación entre **{a[nivel]}** y **{b[nivel]}**:",
        f"- Calidad de cartera: **{dato(a, 'Calidad de Cartera'):,.2f}%** vs. **{dato(b, 'Calidad de Cartera'):,.2f}%**.",
        f"- Distribuidoras en mora: **{_formato_chat_numero(dato(a, 'Distribuidoras en Mora'))}** vs. **{_formato_chat_numero(dato(b, 'Distribuidoras en Mora'))}**.",
        f"- Distribuidoras totales: **{_formato_chat_numero(dato(a, 'Distribuidoras Totales'))}** vs. **{_formato_chat_numero(dato(b, 'Distribuidoras Totales'))}**.",
        f"- Dispersión acumulada: **{_formato_chat_moneda(dato(a, 'Total Dispersado'))}** vs. **{_formato_chat_moneda(dato(b, 'Total Dispersado'))}**.",
    ]

    riesgo_a = dato(a, "Puntaje de Atención")
    riesgo_b = dato(b, "Puntaje de Atención")
    prioridad = a[nivel] if riesgo_a >= riesgo_b else b[nivel]
    lineas.append(
        f"Por la combinación de mora, calidad y variaciones, **{prioridad}** "
        "presenta mayor necesidad de seguimiento."
    )
    return "\n".join(lineas)
...
def _obtener_respuesta_chatbot_vales(
    ...
    """
    Chatbot propio sin API.

    Reconoce preguntas de negocio, hace los cálculos con pandas y responde
    mediante reglas y plantillas auditables.
    """
    ...
    if "SUBDIRECCION" in pregunta_norm:
        nivel = "Subdirección"
    elif "ZONA" in pregunta_norm:
        nivel = "Zona"

    tabla = _agrupar_chatbot_local(df_contexto, nivel=nivel)
    if tabla.empty:
        return f"No pude construir el análisis por {nivel.lower()} con los datos actuales."

    nombres = _buscar_entidades_en_pregunta(pregunta, tabla, nivel)
    ...
    if nombres:
        fila = tabla[
            tabla[nivel].astype(str).map(limpiar_texto) == limpiar_texto(nombres[0])
        ].iloc[0]
        nombre = str(fila[nivel])
        return (
            f"**{nombre}**, al corte **{fecha_sel}**, registra "
            f"**{_formato_chat_numero(fila.get('Distribuidoras Totales', 0))}** distribuidoras, "
            f"de las cuales **{_formato_chat_numero(fila.get('Distribuidoras al Corriente', 0))}** "
            f"están al corriente y **{_formato_chat_numero(fila.get('Distribuidoras en Mora', 0))}** en mora. "
            f"Su calidad de cartera es **{float(fila.get('Calidad de Cartera', 0)):,.2f}%**, "
            f"con dispersión acumulada de **{_formato_chat_moneda(fila.get('Total Dispersado', 0))}** "
            f"y **{_formato_chat_numero(fila.get('Canjes', 0))}** canjes."
        )
    ...
    if "MORA" in pregunta_norm or "ATRASO" in pregunta_norm:
        ...
        if fila is None:
            return "No hay información de mora para responder."
        sentido = "menor" if menor else "mayor"
        return (
            f"La {nivel.lower()} con {sentido} mora es **{fila[nivel]}**, con "
            f"**{_formato_chat_numero(fila.get(columna, 0))}** distribuidoras en mora "
            f"y una calidad de cartera de **{float(fila.get('Calidad de Cartera', 0)):,.2f}%**."
        )
    ...
    if "CALIDAD" in pregunta_norm or "CARTERA SANA" in pregunta_norm:
        ...
        if fila is None:
            return "No hay información de calidad de cartera para responder."
        sentido = "menor" if peor else "mejor"
        return (
            f"La {nivel.lower()} con {sentido} calidad de cartera es **{fila[nivel]}**, "
            f"con **{float(fila.get('Calidad de Cartera', 0)):,.2f}%**; "
            f"tiene **{_formato_chat_numero(fila.get('Distribuidoras al Corriente', 0))}** "
            f"distribuidoras al corriente de **{_formato_chat_numero(fila.get('Distribuidoras Totales', 0))}**."
        )
    ...
    if any(x in pregunta_norm for x in ["DISPERSION", "DISPERSADO", "COLOCADO"]):
        ...
        columna = (
            "Total Dispersado Fecha Corte"
            if any(x in pregunta_norm for x in ["DIA", "CORTE", "HOY"])
            and "Total Dispersado Fecha Corte" in tabla.columns
            else "Total Dispersado"
        )
        fila = _fila_chatbot(tabla, columna, mayor=not menor)
        if fila is None:
            return "No hay información de dispersión para responder."
        alcance = "del día de corte" if columna == "Total Dispersado Fecha Corte" else "acumulada"
        return (
            f"La {nivel.lower()} con {'menor' if menor else 'mayor'} dispersión {alcance} "
            f"es **{fila[nivel]}**, con **{_formato_chat_moneda(fila.get(columna, 0))}**."
        )
    ...
    if "CANJE" in pregunta_norm:
        if "PROMEDIO" in pregunta_norm:
            ...
            columna = "Canjes Fecha Corte"
        else:
            columna = "Canjes"

        menor = any(x in pregunta_norm for x in ["MENOR", "MENOS", "BAJO"])
        fila = _fila_chatbot(tabla, columna, mayor=not menor)
        if fila is None:
            return "No hay información de canjes para responder."

        valor = float(fila.get(columna, 0))
        valor_fmt = _formato_chat_moneda(valor) if columna == "Canje Promedio" else _formato_chat_numero(valor)
        return (
            f"La {nivel.lower()} con {'menor' if menor else 'mayor'} {columna.lower()} "
            f"es **{fila[nivel]}**, con **{valor_fmt}**."
        )
    ...
    if "DISTRIBUIDORA" in pregunta_norm:
        if "CORRIENTE" in pregunta_norm:
            ...
            columna = "Distribuidoras Totales"
        menor = any(x in pregunta_norm for x in ["MENOR", "MENOS", "BAJA"])
        fila = _fila_chatbot(tabla, columna, mayor=not menor)
        if fila is None:
            return "No hay información de distribuidoras para responder."
        return (
            f"La {nivel.lower()} con {'menor' if menor else 'mayor'} número de "
            f"{columna.lower()} es **{fila[nivel]}**, con "
            f"**{_formato_chat_numero(fila.get(columna, 0))}**."
        )
    ...
    if any(x in pregunta_norm for x in ["TOTAL", "RESUMEN", "PANORAMA", "COMO ESTA"]):
        totales = {
            ...
            "mora": _numero_chatbot(df_contexto, "Distribuidoras en Mora").sum(),
            "dispersion": _numero_chatbot(df_contexto, "Total Dispersado").sum(),
            "canjes": _numero_chatbot(df_contexto, "Canjes").sum(),
        }
        calidad = (
            totales["corriente"] / totales["totales"] * 100
            if totales["totales"]
            else 0
        )
        return (
            f"Al corte **{fecha_sel}**, **{nombre_valera}** registra "
            f"**{_formato_chat_numero(totales['totales'])}** distribuidoras: "
            f"**{_formato_chat_numero(totales['corriente'])}** al corriente y "
            f"**{_formato_chat_numero(totales['mora'])}** en mora. "
            f"La calidad de cartera es **{calidad:,.2f}%**. "
            f"La dispersión acumulada es **{_formato_chat_moneda(totales['dispersion'])}**, "
    ...
    return (
        ...
        "o **¿Qué categoría corresponde a $85,000?**"
    )
...
def _dialogo_resumen_con_chatbot(
    ...
    with col_limpiar:
        limpiar = st.button(
            "Limpiar",
            key=f"chat_vales_limpiar_{clave_chat}",
            use_container_width=True,
        )
...
def renderizar_robot_resumen_ejecutivo(
    ...
    """Abre el resumen original y conserva un chatbot local sin API."""
    clave_chat = re.sub(
        r"[^a-zA-Z0-9]+",
        "_",
        f"{nombre_valera}_{fecha_sel}",
    ).strip("_").lower()
